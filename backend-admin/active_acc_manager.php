...
$salesUser = new BackendAdmin($db,$connMongoDb,$strtdate,$enddate);
#get posted data
...
)
{
    ...
    if($rows > 0){
    #set Sales User property values 
    $result_sub = $salesUser->Activeaccmanager();
    $stmt_sub = $result_sub->get_result();
    $rowSub = $stmt_sub->fetch_all(MYSQLI_ASSOC);
    $rowsSub = $stmt_sub->num_rows;
    if($rowsSub > 0){
        # JSON-encode the response
        $json_response = json_encode(array("data"=>$rowSub,"message" => "success"));
        # Return the response
        echo $json_response;
    }
    else{
