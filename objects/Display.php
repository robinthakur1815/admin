...
class Display{
 ...
 #database connection and table name
    ...
    public function getAdtype(){
      ...
      if($table == "mcm_adtypewise_report"){
    ...
    public function getAdunits(){
      ...
      if($this->child_net_code == "demo_9999999"){
           ...
           $queryFetchAdunit = 'SELECT ad_unitwise_date as date,"No Tag" as rep_name,dfp_ad_unit as unit_name,mcm_clicks as clicks, mcm_adreq as adr,mcm_matchreq as madr, mcm_matchreq*100/mcm_adreq as covg, mcm_impression as adimr, mcm_earnings/mcm_impression*1000 as ecpmx,mcm_earnings as revenue FROM mcm_adunitwise_report_demo where child_net_code ="'.$this->child_net_code.'" AND ad_unitwise_date between "'.$this->strtdate.'" and "'.$this->enddate.'" ORDER BY ad_unitwise_date DESC';

      }else{
      if($table == "mcm_adunitwise_report"){
    ...
    public function getSites(){
      ...
      #get memcached class
      $memtest = $this->getMemCache();
      #query#condtion for demo account 
      if($this->child_net_code == "demo_9999999"){
        $queryFetchSites = 'SELECT ad_domain_date as date,site_name,dfp_ad_unit as rep_name,mcm_clicks as clicks, mcm_adreq as adr,mcm_matchreq as madr, mcm_matchreq*100/mcm_adreq as covg, mcm_impression as adimr, mcm_earnings/mcm_impression*1000 as ecpmx,mcm_earnings as revenue FROM mcm_domainwise_report_demo where child_net_code ="'.$this->child_net_code.'" AND ad_domain_date between "'.$this->strtdate.'" and "'.$this->enddate.'" ORDER BY revenue,ad_domain_date,site_name DESC';
      }else{
    ...
    public function getDevice(){
      ...
      if($this->child_net_code == "demo_9999999"){
            ...
            $queryFetchDevice = 'SELECT ad_device_date as date,device_category as device,dfp_ad_unit as rep_name,mcm_clicks as clicks, mcm_adreq as adr,mcm_matchreq as madr, mcm_matchreq*100/mcm_adreq as covg, mcm_impression as adimr, mcm_earnings/mcm_impression*1000 as ecpmx,mcm_earnings as revenue FROM mcm_devicecategory_report_demo where child_net_code ="'.$this->child_net_code.'" AND ad_device_date between "'.$this->strtdate.'" and "'.$this->enddate.'" ORDER BY revenue,ad_device_date,device_category DESC';

         }
      else
      {
      if($table == "mcm_devicecategory_report"){
    ...
    public function getGeo(){
      ...
      if($table == "mcm_countrywise_report"){
