 ...
 #database connection and table name
   ...
   #Get siteurl
    ...
    public function getTrafficSource(){
      ...
      #query
      $where = ['date'=>['$gte' => $this->pstart_week, '$lte' => $this->pend_week], 'account_id'=>$account_id];
        $select_fields  = [];
        $options        = [
            'projection'    => $select_fields,
            'sort'          => ['date' => -1]
	...
	public function getHeatmapUrl(){
		$analyticid = (int)trim($this->account_id);
		$strtdate = trim($this->strtdate);
		$enddate = trim($this->enddate);
		
		$command = new MongoDB\Driver\Command([
			'aggregate' => 'analy_land_exit_two',
			'pipeline' => [
				['$match'=>['account_id'=>$analyticid,'date'=>['$gte' =>$strtdate,'$lte' =>$enddate]]],
				['$group' => ['_id' => '$landingPagePath', 'totalPageViews' => ['$sum' => '$pageviews']]],
				['$sort'=>['totalPageViews'=>-1]],
				['$limit'=>7],
			],
			'cursor' => new stdClass,
		]);
		$cursor = $this->connMongoDb->executeCommand('analytics_hourly', $command);
		return $cursor;
	}
	public function getHeatmapData(){
		...
		$strtdate = trim($this->strtdate);
		$enddate = trim($this->enddate);
		$pagePath = trim($this->pagePath);
		
		$command = new MongoDB\Driver\Command([
			'aggregate' => 'analy_land_exit_two',
			'pipeline' => [
				['$match'=>['account_id'=>$analyticid,'date'=>['$gte' =>$strtdate,'$lte' =>$enddate,],'landingPagePath'=>$pagePath]],
				['$group' => ['_id' => ['date'=>'$date','hour'=>'$hour'],'totalPageViews' => ['$sum' => '$pageviews']]],
				['$sort'=>['_id'=>1]],
			],
			'cursor' => new stdClass,
		]);
