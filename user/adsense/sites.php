...
if(
    ...
    if($rows > 0){
     ...
     $result_site = $adsense->getSites();
     $result_sites = $result_site->toArray();  
     if(!empty($result_sites)){
        #calculation
        $data = prepareData($result_sites,$data->strtdate,$data->enddate);
 ...
 #tell the user data is incomplete
    ...
    #tell the user
    echo json_encode(array("message" => "Unable to get adsense sites. Data is incomplete.","status_code"=>400));
}
function prepareData($result_sites,$start,$end){
#Date Array
while (strtotime($start) <= strtotime($end))
{
 ...
 $start = date ("Y-m-d", strtotime("+1 day", strtotime($start)));
    ...
    foreach ($result_sites as $key => $val) {
        
        $sites = str_replace(".","_",$val->_id->website);
        $dateindex = $val->_id->date;
        #Data merge for first slide by site wise
        @$sumuplevel_array[$sites]['sites']=$sites;
        @$sumuplevel_array[$sites]['adreq']+=$val->totalad_requests;
        @$sumuplevel_array[$sites]['adimpr']+=$val->totalad_imp;
        @$sumuplevel_array[$sites]['madreq']+=$val->totalmatchad_requests;
        @$sumuplevel_array[$sites]['fillrate'] = $sumuplevel_array[$sites]['adreq'] > 0 ? number_format($sumuplevel_array[$sites]['adimpr']/$sumuplevel_array[$sites]['adreq']*100,1):0;
        @$sumuplevel_array[$sites]['clicks']+=$val->total_click;

        @$sumuplevel_array[$sites]['covg'] = $sumuplevel_array[$sites]['madreq'] > 0 ? number_format(($sumuplevel_array[$sites]['madreq']*100)/$sumuplevel_array[$sites]['adreq'],1) :0;

        @$sumuplevel_array[$sites]['ctr'] = $sumuplevel_array[$sites]['adimpr'] > 0 ? number_format($sumuplevel_array[$sites]['clicks']/$sumuplevel_array[$sites]['adimpr']*100,1):0;

        @$sumuplevel_array[$sites]['revenue_cmsShare'] += round($val->total_earning,2);
        ...
        @$arraylevel2[$sites][date('j M', strtotime($val->_id->date))]+=number_format($val->total_earning,2);
        ...
        @$total_array['fillrate']= @$total_array['adreq'] > 0 ? number_format($total_array['adimpr']/$total_array['adreq']*100,1):0;
        @$total_array['clicks']+=$val->total_click;
        @$total_array['covg'] = $total_array['madreq'] > 0 ? number_format(($total_array['madreq']*100)/$total_array['adreq'],1) :0;
        @$total_array['ctr'] = $total_array['adimpr'] > 0 ? number_format($total_array['clicks']/$total_array['adimpr']*100,1):0;
        ...
        @$total_array['ecpm'] = $total_array['adimpr'] > 0 ? number_format($total_array_full['revenue_cmsShare']/$total_array['adimpr']*1000,2) : 0;

    #first table page inner
    @$datalevel1inner[$sites][$dateindex]['dateinner']= date('j M, Y', strtotime($val->_id->date));
    @$datalevel1inner[$sites][$dateindex]['adrinner']+=$val->totalad_requests;
    @$datalevel1inner[$sites][$dateindex]['adimrinner']+=$val->totalad_imp;
    @$datalevel1inner[$sites][$dateindex]['madrinner']+=$val->totalmatchad_requests;
    @$datalevel1inner[$sites][$dateindex]['clicksinner']+=$val->total_click;
    @$datalevel1inner[$sites][$dateindex]['covginner'] = $datalevel1inner[$sites][$dateindex]['madrinner'] > 0 ? number_format(($datalevel1inner[$sites][$dateindex]['madrinner']*100)/$datalevel1inner[$sites][$dateindex]['adrinner'],1) :0;
    @$datalevel1inner[$sites][$dateindex]['ctrinner']+=$datalevel1inner[$sites][$dateindex]['adimrinner'] > 0 ? number_format($datalevel1inner[$sites][$dateindex]['clicksinner']/$datalevel1inner[$sites][$dateindex]['adimrinner']*100,1):0;
    @$datalevel1inner[$sites][$dateindex]['revenue_cmsShareinner']+=round($val->total_earning,2);  
    @$datalevel1inner[$sites][$dateindex]['ecpmxinner'] =$datalevel1inner[$sites][$dateindex]['adimrinner'] > 0 ? number_format($datalevel1inner[$sites][$dateindex]['revenue_cmsShareinner']/$datalevel1inner[$sites][$dateindex]['adimrinner']*1000,2) : 0.00; 
...
foreach($array_fulldate as $date_value)
{
    ...
    { 
        ...
        'fillrate'=> @$array_data[$date_value]['adrinner'] > 0 ? number_format(@$array_data[$date_value]['adimrinner']/@$array_data[$date_value]['adrinner']*100,1):0,
