...
if(
    ...
    if(!empty($result_hb)){
        ...
        #calculation
        $data = prepareData($result_hb,$cmsShare,$data->uniq_id);
...
function prepareData($result_hb,$cmsShare,$uniqid){
...
$filename = "CyberAds_Pro_Report_Device_".$uniqid.".csv";
...
foreach($sumuplevel_array as $row=>$value)    {
    ...
    $header=array("--Device--","--Total Imp.--","--Estimated CPM--","--Estimated Earnings--");
    fputcsv($fp,$header);
