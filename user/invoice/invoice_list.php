...
if(
    ...
    if($rows > 0){
        ...
        $pubInvoice->month_year = date('F Y', strtotime('-40 days'));
        $pubInvoice->curr_month_year = date('F Y');
        $result_inv = $pubInvoice->getInvoices();
        ...
        if(!empty($bank_details)){
			if(!empty($result_inv)){
				...
				$json_response = json_encode(array("invoicing"=>$result_inv,"message" => "success","status_code"=>200));
				# Return the response
				echo $json_response;
			}else{
