...
function prepareData($cmsShare,$result_sites,$uniqid){
    ...
    foreach ($result_sites as $key => $rowsite) {
        ...
        @$sumuplevel_array[$sites]['covg'] = $sumuplevel_arraymad[$sites]['madreq'] > 0 ? number_format(($sumuplevel_arraymad[$sites]['madreq']*100)/$sumuplevel_array[$sites]['adreq'],1) :0.00;
        @$sumuplevel_array[$sites]['ctr'] = $sumuplevel_array[$sites]['adimpr'] > 0 ? number_format($click_array[$sites]['clicks']/$sumuplevel_array[$sites]['adimpr']*100,1):0.00;
    ...
    @$datalevel1inner[$sites][$dateindex]['dateinner']= date('d M, Y', strtotime($rowsite['date']));
    ...
    @$datalevel1inner[$sites][$dateindex]['covginner'] = $datalevel1innermad[$sites][$dateindex]['madrinner'] > 0 ? number_format(($datalevel1innermad[$sites][$dateindex]['madrinner']*100)/$datalevel1inner[$sites][$dateindex]['adrinner'],1) :0.00;
    @$datalevel1inner[$sites][$dateindex]['ctrinner']+=$datalevel1inner[$sites][$dateindex]['adimrinner'] > 0 ? number_format($clicklevel1inner[$sites][$dateindex]['clicksinner']/$datalevel1inner[$sites][$dateindex]['adimrinner']*100,1):0.00;
    @$revenuelevel1inner[$sites][$dateindex]['revenue_cmsShareinner']+=round($rowsite['revenue']-($rowsite['revenue']*$cmsShare),2);  
    @$datalevel1inner[$sites][$dateindex]['ecpmxinner'] =$datalevel1inner[$sites][$dateindex]['adimrinner'] > 0 ? number_format(floor(($revenuelevel1inner[$sites][$dateindex]['revenue_cmsShareinner']/$datalevel1inner[$sites][$dateindex]['adimrinner']*1000)*100)/100, 2) : 0.00;
    ...
    @$sumuplevel2_array[$sites][$repname]['covglvl1'] = $sumuplevel2_array[$sites][$repname]['madrlvl1'] > 0 ? number_format(($sumuplevel2_array[$sites][$repname]['madrlvl1']*100)/$sumuplevel2_array[$sites][$repname]['adrlvl1'],1) :0.00;    
    @$sumuplevel2_array[$sites][$repname]['ctrlvl1'] = $sumuplevel2_array[$sites][$repname]['adimrlvl1'] > 0 ? number_format($click2_array[$sites][$repname]['clickslvl1']/$sumuplevel2_array[$sites][$repname]['adimrlvl1']*100,1):0.00;
    @$rev2_array[$sites][$repname]['revenue_cmsSharelvl1']+=round($rowsite['revenue']-($rowsite['revenue']*$cmsShare),2);
    @$sumuplevel2_array[$sites][$repname]['ecpmxlvl1'] = $sumuplevel2_array[$sites][$repname]['adimrlvl1'] > 0 ? number_format(floor(($rev2_array[$sites][$repname]['revenue_cmsSharelvl1']/$sumuplevel2_array[$sites][$repname]['adimrlvl1']*1000)*100)/100, 2) : 0.00;
...
$filename = "Auxo_Network_Report_App_Sites_".$uniqid.".csv";
...
foreach($sumuplevel_array as $row=>$value)   {
    ...
    $header=array("--App--","--Ad Requests--","--Ad Impressions--","--Fill Rate--","--Coverage--","--CTR--","--Estimated eCPM--","--Estimated Earnings--");
    fputcsv($fp,$header);
    ...
    fputcsv($fp,$blank);
    $header=array("--App--","--Ad Units--","--Ad Requests--","--Fill Rate--","--Matched Ad Request--","--Coverage--","--CTR--","--Estimated eCPM--","--Estimated Earnings--");
    fputcsv($fp,$header);
