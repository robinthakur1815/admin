...
if(
     ...
     if(!empty($rowShare)){if($rowShare['pub_app_share'] !=0){$cmsShare = $rowShare['pub_app_share']/100;}else{$cmsShare = 15/100;}}else{$cmsShare = 15/100;} 
		...
		if(!empty($resultDatalast20_app)){
			foreach ($resultDatalast20_app as $key2 => $val2) {
				...
				@$last_lastData['ad_exch_revenue'] += round(($val2['ad_exch_revenue'])-($val2['ad_exch_revenue']*$cmsShare),2);
		...
		if(!empty($resultDatalast10_app)){
			foreach ($resultDatalast10_app as $key8 => $val8) {
				...
				@$last_Data['ad_exch_revenue'] += round(($val8['ad_exch_revenue'])-($val8['ad_exch_revenue']*$cmsShare),2);
				...
				@$testDataCheck[$val8['ad_exch_date']]['ad_exch_revenue'] = round(($val8['ad_exch_revenue'])-($val8['ad_exch_revenue']*$cmsShare),2);
	...
	if(!empty($result_app)){
          ...
          #calculation
          $dataTable = prepareData($cmsShare,$result_app,$data->strtdate,$data->enddate);
...
function prepareData($cmsShare,$result_app,$start,$end){
...
foreach($date_array as $date_value)
    {
     if (in_array($date_value, $dateavail))
            {
                $final_sum_array['app'][$date_value]=$revenue[$date_value];
            }
            else
            {
                $final_sum_array['app'][$date_value]=0;
            }
    }
   
foreach(array_reverse($final_sum_array['app']) as $keyads=>$valueads)
    {
