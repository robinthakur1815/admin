...
if(
    ...
    if($rows > 0){
		...
		if(!empty($result_display)){
			#calculation
			$data = prepareData($cmsShare,$result_display,$data->uniq_id);
...
function prepareData($cmsShare,$result_display,$uniqid){
	...
	foreach ($result_display as $key => $rowcontent) {
        ...
        @$total_array['covg'] = $total_array_down['madr'] > 0 ? number_format(($total_array_down['madr']*100)/$total_array['adr'],1) :0.0;
        @$total_array['ctr'] = $total_array['adimr'] > 0 ? number_format($total_array_cli['clicks']/$total_array['adimr']*100,1):0.0;
	...
	$filename = "Auxo_Network_Report_Display_Overview_".$uniqid.".csv";
