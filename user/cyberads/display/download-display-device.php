...
function prepareData($cmsShare,$result_device,$uniqid){
    ...
    foreach ($result_device as $key => $rowdevice) {
        ...
        @$sumuplevel_array[$device]['covg'] = $sumuplevel_arraymad[$device]['madreq'] > 0 ? number_format(($sumuplevel_arraymad[$device]['madreq']*100)/$sumuplevel_array[$device]['adreq'],1) :0.0;
        @$sumuplevel_array[$device]['ctr'] = $sumuplevel_array[$device]['adimpr'] > 0 ? number_format($click_array[$device]['clicks']/$sumuplevel_array[$device]['adimpr']*100,1):0.0;
        @$revenue_array[$device]['revenue_cmsShare'] += round($rowdevice['revenue']-($rowdevice['revenue']*$cmsShare),2);
        @$sumuplevel_array[$device]['ecpm'] = $sumuplevel_array[$device]['adimpr'] > 0 ? number_format($revenue_array[$device]['revenue_cmsShare']/$sumuplevel_array[$device]['adimpr']*1000,2) : 0.00;
    ...
    @$datalevel1inner[$device][$dateindex]['covginner'] = $datalevel1innermad[$device][$dateindex]['madrinner'] > 0 ? number_format(($datalevel1innermad[$device][$dateindex]['madrinner']*100)/$datalevel1inner[$device][$dateindex]['adrinner'],1) :0.0;
    @$datalevel1inner[$device][$dateindex]['ctrinner']+=$datalevel1inner[$device][$dateindex]['adimrinner'] > 0 ? number_format($clicklevel1inner[$device][$dateindex]['clicksinner']/$datalevel1inner[$device][$dateindex]['adimrinner']*100,1):0.0;
     @$revenuelevel1inner[$device][$dateindex]['revenue_cmsShareinner']+=round($rowdevice['revenue']-($rowdevice['revenue']*$cmsShare),2); 
    @$datalevel1inner[$device][$dateindex]['ecpmxinner'] =$datalevel1inner[$device][$dateindex]['adimrinner'] > 0 ? number_format($revenuelevel1inner[$device][$dateindex]['revenue_cmsShareinner']/$datalevel1inner[$device][$dateindex]['adimrinner']*1000,2) : 0.00;
    ...
    @$sumuplevel2_array[$device][$repname]['covglvl1'] = $sumuplevel2_arraymad[$device][$repname]['madrlvl1'] > 0 ? number_format(($sumuplevel2_arraymad[$device][$repname]['madrlvl1']*100)/$sumuplevel2_array[$device][$repname]['adrlvl1'],1) :0.0;    
    @$sumuplevel2_array[$device][$repname]['ctrlvl1'] = $sumuplevel2_array[$device][$repname]['adimrlvl1'] > 0 ? number_format($click2_array[$device][$repname]['clickslvl1']/$sumuplevel2_array[$device][$repname]['adimrlvl1']*100,1):0.0;
    @$rev2_array[$device][$repname]['revenue_cmsSharelvl1']+=round($rowdevice['revenue']-($rowdevice['revenue']*$cmsShare),2);
    @$sumuplevel2_array[$device][$repname]['ecpmxlvl1'] = $sumuplevel2_array[$device][$repname]['adimrlvl1'] > 0 ? number_format($rev2_array[$device][$repname]['revenue_cmsSharelvl1']/$sumuplevel2_array[$device][$repname]['adimrlvl1']*1000,2) : 0.00;
...
$filename = "Auxo_Network_Report_Display_Device_".$uniqid.".csv";
