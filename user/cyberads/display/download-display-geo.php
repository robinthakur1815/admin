...
function prepareData($cmsShare,$result_geo,$uniqid,$start,$end){
    ...
    foreach ($result_geo as $key => $rowgeo) {
        ...
        @$sumuplevel_array[$geo]['covg'] = $sumuplevel_arraymad[$geo]['madreq'] > 0 ? number_format(($sumuplevel_arraymad[$geo]['madreq']*100)/$sumuplevel_array[$geo]['adreq'],1) :0.0;
        @$sumuplevel_array[$geo]['ctr'] = $sumuplevel_array[$geo]['adimpr'] > 0 ? number_format($click_array[$geo]['clicks']/$sumuplevel_array[$geo]['adimpr']*100,1):0.0;
        @$revenue_array[$geo]['revenue_cmsShare'] += round($rowgeo['revenue']-($rowgeo['revenue']*$cmsShare),2);
        @$sumuplevel_array[$geo]['ecpm'] = $sumuplevel_array[$geo]['adimpr'] > 0 ? number_format($revenue_array[$geo]['revenue_cmsShare']/$sumuplevel_array[$geo]['adimpr']*1000,2) : 0.00;
    ...
    @$datalevel1inner[$geo][$dateindex]['covginner'] = $datalevel1innermad[$geo][$dateindex]['madrinner'] > 0 ? number_format(($datalevel1innermad[$geo][$dateindex]['madrinner']*100)/$datalevel1inner[$geo][$dateindex]['adrinner'],1) :0.0;
    @$datalevel1inner[$geo][$dateindex]['ctrinner']+=$datalevel1inner[$geo][$dateindex]['adimrinner'] > 0 ? number_format($clicklevel1inner[$geo][$dateindex]['clicksinner']/$datalevel1inner[$geo][$dateindex]['adimrinner']*100,1):0.0;
    @$revenuelevel1inner[$geo][$dateindex]['revenue_cmsShareinner']+=round($rowgeo['revenue']-($rowgeo['revenue']*$cmsShare),2);  
    @$datalevel1inner[$geo][$dateindex]['ecpmxinner'] =$datalevel1inner[$geo][$dateindex]['adimrinner'] > 0 ? number_format($revenuelevel1inner[$geo][$dateindex]['revenue_cmsShareinner']/$datalevel1inner[$geo][$dateindex]['adimrinner']*1000,2) : 0.00;
    ...
    @$sumuplevel2_array[$geo][$repname]['ecpmxlvl1'] = $sumuplevel2_array[$geo][$repname]['adimrlvl1'] > 0 ? number_format($rev2_array[$geo][$repname]['revenue_cmsSharelvl1']/$sumuplevel2_array[$geo][$repname]['adimrlvl1']*1000,2) : 0.00;
                  ...
                  foreach($sumOtherInner as $level1suminner)
                    {
                        foreach($level1suminner as $level2key=>$level2values)
                        {
                            ...
                            @$finalinnerdatalevel1[$finalindex]['ecpmxinner'] =$finalinnerdatalevel1[$finalindex]['adimrinner'] > 0 ? number_format(($finalinnerdata_rev[$finalindex]['revenue_cmsShareinner'])/$finalinnerdatalevel1[$finalindex]['adimrinner']*1000,2) :0.00;
...
$filename = "Auxo_Network_Report_Display_Geo_".$uniqid.".csv";
