...
$dashSites = new DashboardAuxo($db);
#get posted data
...
if(
    ...
    if($rows > 0){
     ...
     $dashSites->range = $data->range;
     $dashSites->strtdate = $data->strtdate;
     $dashSites->enddate = $data->enddate;
     $dashSites->child_net_code = $data->child_net_code;
     $result_sites = $dashSites->getSites();
   
     if(!empty($result_sites['Display']) || !empty($result_sites['App']) || !empty($result_sites['Video'])){
        #calculation
        $data = prepareData($cmsShare,$cmsShareApp,$cmsShareVid,$result_sites,$data->strtdate,$data->enddate);
 ...
 #tell the user data is incomplete
    ...
    #tell the user
    echo json_encode(array("message" => "Unable to get dashboard sites. Data is incomplete.","status_code"=>400));
}
function prepareData($cmsShare,$cmsShareApp,$cmsShareVid,$result_sites,$start,$end){
     #Date Array
    ...
    foreach ($result_sites as $key => $rowsites) {
        ...
        $dateindex = date('Y-m-j', strtotime($rowsite['date']));
         ...
         if($key == 'Display'){
               ...
               @$arraylevel2[$sites][$repname]+=round($rowsite['revenue']-($rowsite['revenue']*$cmsShare),2);
        ...
        if($key == 'App'){
               @$sumuplevel_array[$sites]['revenue_cmsShare'] += round($rowsite['revenue']-($rowsite['revenue']*$cmsShareApp),2);
               @$arraylevel2[$sites][$repname]+=round($rowsite['revenue']-($rowsite['revenue']*$cmsShareApp),2);
               @$sumuplevel2_array[$sites][$repname]['revenue_cmsSharelvl1']+=round($rowsite['revenue']-($rowsite['revenue']*$cmsShareApp),2);
                #inner
                @$datalevel2inner[$sites][$repname][$dateindex]['dateinnerlvl1']= date('j M, Y', strtotime($rowsite['date']));
                @$datalevel2inner[$sites][$repname][$dateindex]['revenue_cmsShareinnerlvl1']+=round($rowsite['revenue']-($rowsite['revenue']*$cmsShareApp),2);
         }
         if($key == 'Video'){
               @$sumuplevel_array[$sites]['revenue_cmsShare'] += round($rowsite['revenue']-($rowsite['revenue']*$cmsShareVid),2);
               @$arraylevel2[$sites][$repname]+=round($rowsite['revenue']-($rowsite['revenue']*$cmsShareVid),2);
               @$sumuplevel2_array[$sites][$repname]['revenue_cmsSharelvl1']+=round($rowsite['revenue']-($rowsite['revenue']*$cmsShareVid),2);
                #inner
                @$datalevel2inner[$sites][$repname][$dateindex]['dateinnerlvl1']= date('j M, Y', strtotime($rowsite['date']));
                @$datalevel2inner[$sites][$repname][$dateindex]['revenue_cmsShareinnerlvl1']+=round($rowsite['revenue']-($rowsite['revenue']*$cmsShareVid),2);
         }
...
foreach($sumuplevel2_array as $keyfinal2=>$valuelvl1)
    ...
    foreach($valuelvl1 as $value2)
      ...
      $response_array1['sites_table_data_lvl1']=array_values($dataarray_array['sites_table_data_lvl1']);  
       /***level 3***/
      $mar = array(
         "enabled"=> false,
          "symbol"=> "circle");
...
foreach($response_array1['sites_table_data_lvl1'] as $lvl3){
    ...
    foreach($lvl3['innerdatalvl1'] as $lvl3data){
          ...
          $response_array['level3'][]=array(
                                                'name'=>$lvl3['level1value']."-".$lvl3['rep_namelvl1'],
...
$response_array['finaldata']=array_merge($response_array1['level2'],$response_array['level3']);
