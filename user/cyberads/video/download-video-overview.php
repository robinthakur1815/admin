...
if(
    ...
    if($rows > 0){
     ...
     if(!empty($result_video)){
          ...
          #calculation
          $data = prepareData($cmsShare,$result_video,$data->uniq_id,$data->strtdate,$data->enddate);
...
function prepareData($cmsShare,$result_video,$uniqid){
    ...
    foreach ($result_video as $key => $rowcontent) {
        ...
        @$sumuplevel_array_rev[$date]['revenue_cmsShare'] += $rowcontent['revenue']-($rowcontent['revenue']*$cmsShare);
        ...
        @$total_array['ctr'] = $total_array['adimr'] > 0 ? number_format($total_array_cli['clicks']/$total_array['adimr']*100,1):0.00;
        @$total_array_full['revenue_cmsShare'] += round(($rowcontent['revenue']-($rowcontent['revenue']*$cmsShare)),2);
    ...
    $request_array['content_table_data'][] = array("date"=>"Total","adr"=>$total_array['adr'],"adimr"=>$total_array['adimr'],"fillrate"=>$total_array['fillrate'],"covg"=>$total_array['covg'],"ctr"=>$total_array['ctr'],"ecpm"=>$total_array['ecpm'],"revenue_cmsShare"=>$total_array['revenue_cmsShare']);
    
    
    $filename = "Auxo_Network_Report_Video_Overview_".$uniqid.".csv";
