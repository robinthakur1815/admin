...
function prepareData($cmsShare,$result_adunit,$uniqid){
    ...
    foreach ($result_adunit as $key => $rowadunit) {
        ...
        @$sumuplevel_array[$unitName]['covg'] = $sumuplevel_arraymad[$unitName]['madreq'] > 0 ? number_format(($sumuplevel_arraymad[$unitName]['madreq']*100)/$sumuplevel_array[$unitName]['adreq'],1) :0.00;
        @$sumuplevel_array[$unitName]['ctr'] = $sumuplevel_array[$unitName]['adimpr'] > 0 ? number_format($click_array[$unitName]['clicks']/$sumuplevel_array[$unitName]['adimpr']*100,1):0.00;
        @$revenue_array[$unitName]['revenue_cmsShare'] += round($rowadunit['revenue']-($rowadunit['revenue']*$cmsShare),2);
        @$sumuplevel_array[$unitName]['ecpm'] = $sumuplevel_array[$unitName]['adimpr'] > 0 ? number_format(floor(($revenue_array[$unitName]['revenue_cmsShare']/$sumuplevel_array[$unitName]['adimpr']*1000)*100)/100, 2) : 0.00;
        @$sumuplevel_array[$unitName]['revenue_cmsShare'] = $revenue_array[$unitName]['revenue_cmsShare'];
        

        #inner
    @$datalevel2inner[$unitName][$dateindex]['dateinnerlvl1']= date('d M, Y', strtotime($rowadunit['date']));
    ...
    @$datalevel2inner[$unitName][$dateindex]['covginnerlvl1'] = $datalevel2innermad[$unitName][$dateindex]['madrinnerlvl1'] > 0 ? number_format(($datalevel2innermad[$unitName][$dateindex]['madrinnerlvl1']*100)/$datalevel2inner[$unitName][$dateindex]['adrinnerlvl1'],1) :0.00;
    @$datalevel2inner[$unitName][$dateindex]['ctr']+=$datalevel2inner[$unitName][$dateindex]['adimrinnerlvl1'] > 0 ? number_format($clicks_inner[$unitName][$dateindex]['clicksinnerlvl1']/$datalevel2inner[$unitName][$dateindex]['adimrinnerlvl1']*100,1):0.00;
    @$revenue2inner[$unitName][$dateindex]['revenue_cmsShareinnerlvl1']+=round($rowadunit['revenue']-($rowadunit['revenue']*$cmsShare),2);  
    @$datalevel2inner[$unitName][$dateindex]['ecpmxinnerlvl1'] =$datalevel2inner[$unitName][$dateindex]['adimrinnerlvl1'] > 0 ? number_format(floor(($revenue2inner[$unitName][$dateindex]['revenue_cmsShareinnerlvl1']/$datalevel2inner[$unitName][$dateindex]['adimrinnerlvl1']*1000)*100)/100, 2) : 0.00;
...
$filename = "Auxo_Network_Report_Video_AdUnits_".$uniqid.".csv";
