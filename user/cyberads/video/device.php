...
if(
 ...
 #tell the user data is incomplete
    ...
    #tell the user
    echo json_encode(array("message" => "Unable to get video device. Data is incomplete.","status_code"=>400));
}
function prepareData($cmsShare,$result_device,$start,$end){
    ...
    foreach ($result_device as $key => $rowdevice) {
      ...
      @$arraylevel3[$device][$rowdevice['rep_name']][$rowdevice['date']]+=number_format($rowdevice['revenue']-($rowdevice['revenue']*$cmsShare),2);
        ...
        @$total_array['fillrate']=number_format($total_array['adimpr']/$total_array['adreq']*100,2);
    ...
    @$sumuplevel2_array[$device][$repname]['fillrate'] = number_format($sumuplevel2_array[$device][$repname]['adimrlvl1']/$sumuplevel2_array[$device][$repname]['adrlvl1']*100,2);
