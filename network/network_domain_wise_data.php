...
if(
    ...
    if($rows > 0){
     ...
     $result_sites = $network->getNetworkAdsData("domain");

     if(!empty($result_sites)){
        $data = $result_sites;
		//$data = prepareData($cmsShare,$result_sites,$data->strtdate,$data->enddate);
        http_response_code(200);
        echo json_encode(array("message" => "Data Found!","data"=>$data,"status_code"=>200));
      }else{
    ...
    echo json_encode(array("message" => "Invalid Request. Data is incomplete.","status_code"=>400));
}
...
function prepareData($cmsShare,$result_sites,$start,$end){
    ...
    foreach ($result_sites as $key => $rowsite) {
		...
		@$sumuplevel_array[$sites]['date']=$dateindex;
