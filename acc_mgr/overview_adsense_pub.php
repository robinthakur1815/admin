...
if(
    ...
    if($rows > 0){
       ...
       $accmgr->date = $Date;
       $result_topPub = $accmgr->top20AdsData();
       $result_inactivePub = $accmgr->inactiveAdsData();
       $result_adsMoversPub = $accmgr->moversAdsData();
       if(!empty($result_topPub) || !empty($result_inactivePub) ){
            $CurrentDate = date('Y-m-d', strtotime("-1 day", strtotime($Date)));
            # JSON-encode the response
            $json_response = json_encode(array("data"=>array("Top20DateAds"=>date('j F, Y', strtotime($CurrentDate)),"Top20"=>$result_topPub,"ads_inactive"=>$result_inactivePub,"ads_movers"=>$result_adsMoversPub,"AdsMoversDate"=>date('j F, Y', strtotime($CurrentDate))),"status_code"=>200));
